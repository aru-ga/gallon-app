...
const reqChangeAddress = async (address: object) => {
    ...
    try {
        const response = await axios.patch(`${APIS_URL}/profile`, { address }, {
            headers: {
...
const reqChangePassword = async (oldPassword: string, newPassword: string) => {
  ...
  const updateProfilePicture = async (imageFile: File): Promise<any> => {
    const token = localStorage.getItem("authToken");
    ...
    formData.append("profile_picture_url", imageFile);
  ...
  const updateProfile = async (profile: any) => {
    ...
    try {
      const response = await axios.patch(`${APIS_URL}/profile`, { ...profile }, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
export { reqChangePassword, reqChangeAddress, updateProfilePicture, updateProfile };